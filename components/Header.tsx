        ...
        <nav className="hidden lg:flex items-center gap-4 xl:gap-6">
          {siteConfig.navItems.map((item) => (
            <button
              ...
              className={`text-xs lg:text-sm font-medium whitespace-nowrap transition-colors hover:text-blue-600 ${
                isActive(item.href, item.matchPrefix) ? 'text-blue-600' : 'text-slate-600'
              }`}
            >
              {item.label}
            </button>
          ...
          {siteConfig.navItems.map((item) => (
            <button
              ...
              className={`text-left text-sm font-medium p-2 rounded hover:bg-slate-50 transition-colors ${
                isActive(item.href, item.matchPrefix) ? 'text-blue-600 bg-blue-50' : 'text-slate-600'
              }`}
            >
              {item.label}
            </button>
